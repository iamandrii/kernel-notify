...
pub struct Chained<'a, I1, O1, E1, O2, E2> {
    a: Box<dyn Task<'a, Input = I1, Output = O1, Error = E1> + 'a>,
    b: Box<dyn Task<'a, Input = O1, Output = O2, Error = E2> + 'a>,
}

impl<'a, I1, O1, E1, O2, E2> Task<'a> for Chained<'a, I1, O1, E1, O2, E2>
where
    I1: Send + Sync + 'a,
    O1: Send + Sync + 'a,
    E1: Send + Sync + 'a,
    O2: Send + Sync + 'a,
    E2: Send + Sync + From<E1> + 'a,
{
    type Input = I1;
    type Output = O2;
    type Error = E2;
...
impl<'a, I1, O1, E1, O2, E2> Chained<'a, I1, O1, E1, O2, E2>
where
    I1: Send + Sync + 'a,
    O1: Send + Sync + 'a,
    E1: Send + Sync + 'a,
